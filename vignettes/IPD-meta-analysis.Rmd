...
samples_all <- list()
ipd_all <- list()
for(i in 1:length(ds$studyid)){
  ds_each <- ds[ds$studyid == i,] 
  ipd_all[[i]] <- with(ds_each, ipdnma.model.twostage.first(y = y, treat = treat, X = cbind(z1, z2), response = "normal", shrinkage = "laplace"))
  samples_all[[i]] <- ipd.run(ipd_all[[i]])
}
ipd2 <- ipdnma.model.twostage.second(ipd_all, samples_all)
samples2 <- ipd.run(ipd2)


bb <- unstandardize_coefficients(ipd_all[[1]], samples_all[[1]])

ds_each <- ds[ds$studyid == i,] 
ipd_all[[i]] <- with(ds_each, ipdnma.model.twostage.first(y = y, treat = treat, X = cbind(z1, z2), response = "normal", shrinkage = "laplace", scale = FALSE))
samples_all[[i]] <- ipd.run(ipd_all[[i]])
```
